...
- Smart DCA: `~/.openclaw/workspace/configs/dca/<TOKEN>.json` (one per token)
- Basic DCA: `~/.openclaw/workspace/configs/dca/<plan_id>.json`
...
- `idle_yield.position_id`, if present, parks idle USDC in a stable pool for yield. Cron withdraws per-buy amount each cycle before executing the swap.
