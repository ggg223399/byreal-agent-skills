...
- For user-relative windows, run `python3 skills/byreal-predict/scripts/sports-time.py filter --timezone "<timezone>" --date <YYYY-MM-DD> --kind <tonight|today> --fixtures-json '<json array>'` after collecting source fixtures. Render the helper's `count`, `kickoff_utc`, and `your_time` fields. Do not manually recalculate or rename them.
