...
## User-Facing Output Gate

Apply this gate before every reply from this skill.

- Output only the final user-facing answer. Do not leak analysis, tool plans, internal file names, rule citations, "The user wants", "Let me", `AGENTS.md`, `SKILL.md`, `sports-timing.md`, raw JSON, or `<mm:think>` tags.
- Trade, funding, account, position, and order replies use compact Markdown lines, not Markdown tables and not fenced code blocks.
- Truncate all addresses and identifiers that are useful to show. Full wallet, proxy, deposit, order, condition, and token addresses stay hidden unless the user explicitly asks for the full value.
- Unsupported order types reply only with `Unsupported order type shape`; do not search, quote, show holdings, suggest market/limit substitutes, or ask a follow-up.
- For validation blockers such as minimum size, balance, tick size, precision, missing proxy, or missing wallet, state the blocker and stop. Do not add a "found it" preface, market summary, replacement order, or choice list.
...
- **User-facing output gate is mandatory**: never expose hidden reasoning, internal rule/file names, raw tool thoughts, or `<mm:think>` tags; never render trade/funding/account replies as tables or fenced code blocks.
...
- **Limit order semantics**: buy limit price is the maximum acceptable price and may fill at that price or lower; sell limit price is the minimum acceptable price and may fill at that price or higher. For BUY, a higher limit is looser and permits paying more; a lower executable price is better. Never say a buy limit above the current ask must wait for the market to rise, never call it beneficial/favorable, and never call a worse buy price better.
